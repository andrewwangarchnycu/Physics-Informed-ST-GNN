...
\begin{figure}[htbp]
    ...
    \caption{V4 擴大研究區域之環境部 IoT 微型感測器空間分布實測圖（區域內逐時觀測，2025 年夏季正午時段）}
...
\begin{figure}[htbp]
    ...
    \caption{V4 研究範圍選定：擴大區域之 IoT 感測器密度曲面與 300 個選定真實場景對照}
    \label{fig:studyarea}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{300 個真實訓練場景中之 25 個代表性場景（$5\times5$ 陣列）：真實 OSM 建物足跡（色階＝簷高）與 ETH 樹冠。各子圖標題示場景編號、建物棟數與所屬月份。}
...
\begin{figure}[htbp]
    ...
    \caption{熱舒適訓練資料之逐時生成：四個真實場景（列）於 08:00--18:00 各代表時步（行）之模擬 UTCI 場，灰色為建物量體。展現熱壓力之日間動態與真實幾何遮蔽所致之空間梯度。}
...
建物幾何之萃取僅處理封閉方式（Closed Way）——即端點座標相同、由 OSM 節點座標依序連接而成之多邊形——此涵蓋研究區域內絕大多數建物；由關係（Relation）組成之少數大型建物複合體多邊形未納入處理範圍，如實計入未擷取項目，而非以近似輪廓填補。建物樓高之判讀依序嘗試 \texttt{height}、\texttt{building:height} 標籤，其次以 \texttt{building:levels} 樓層數乘以標準層高（3.6 公尺）推算；當標籤含分號分隔之多值（如合併建物之 \texttt{building:levels="11;10;12"}，反映該建物實際由不同樓高之區塊合併登錄）時，取各值之平均。當上述來源皆缺失時，以三層樓（10.8 公尺）之預設值回填，並以布林旗標誠實標記該筆高度之真實性——僅當高度直接來自 OSM 標籤時計為真實，回填值不計入。此一設計使後續統計得以誠實回報：研究區域內約 14\% 建物具真實樓高標籤，其餘以樓層預設值回填，此為本資料集之已知限制（第~\ref{subsec:v4_real_dataset}節）。
...
\begin{figure}[htbp]
    ...
    \caption{OSM 地表鋪面與材質圖資擷取：(a) 原始材質多邊形與建物足跡；(b) 柵格化 land\_cover\_map。}

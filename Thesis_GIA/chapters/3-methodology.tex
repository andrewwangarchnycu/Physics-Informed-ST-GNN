...
\begin{enumerate}
    \item \textbf{模組一：基本 GIS 資訊建構}（GIS Foundation Construction）\\
    以國土測繪中心（NLSC）WMTS／WMS 圖資服務與環境部智慧城鄉 IoT 微氣象感測網路為基礎，建立研究場域之座標參考系統、建物量體資料與感測器空間分布，作為後續模擬與驗證之地理錨點。
    \item \textbf{模組二：都市形態模擬}（Urban Morphology Simulation）\\
    以 Ladybug Tools／EnergyPlus 於 Rhino Grasshopper 環境中執行全場域微氣候逐時模擬，模擬對象涵蓋隨機生成之訓練場景與真實擷取之都市量體兩類幾何輸入，取得作為機器學習訓練監督信號之熱壓力目標值。
    \item \textbf{模組三：運算框架 —— 真實都市建物足跡與樓高擷取}（Real-World Building Footprint \& Height Extraction）\\
    自 NLSC 多重圖資服務融合擷取真實建物輪廓與樓層資訊，取代傳統仰賴蒙地卡羅隨機採樣之單一隨機幾何生成方式，使模型驗證得以錨定於實際都市紋理。
    \item \textbf{模組四：異質圖建構與 PI-ST-GNN 推理}（Heterogeneous Graph Construction \& Inference）\\
    將場景幾何轉譯為包含建物節點與空氣評估節點之異質圖，並以物理資訊時空圖神經網路低延遲預測全場域 UTCI 熱壓力場。
...
\begin{figure}[htbp]
    ...
    \caption{研究場域地理脈絡：由國家尺度逐層放大至場域尺度}
    \label{fig:site_context}
\end{figure}

上圖 (a) 為台灣全島輪廓與新竹市相對位置；(b) 為新竹市東區之 IoT 感測器（Moenv）與 CWA 氣象站實際分布，橘色框標示研究區域範圍；(c) 為 $80\times80$ 公尺研究場域之典型隨機生成場景配置，展示建築量體、退縮帶與 $79\times79$ 個空氣評估節點格點。

\subsection{座標參考系統與圖資來源}
\label{subsec:crs_sources}

研究場域座標統一於 Web Mercator 投影（EPSG:3857）中運算，並於輸出階段轉換回研究區域慣用之 WGS84 地理座標（EPSG:4326）供跨系統交換。所有底圖圖資取自 NLSC 提供之三項服務：

\begin{itemize}
    \item \textbf{TOPO01K}（1/1000 都市計畫現況圖，WMTS）：以粉紅色封閉曲線標繪建物外框，內部以細斜線 hatch 填充樓地板範圍，並於各獨立樓高分區內標註樓層文字（如 \texttt{3R}、\texttt{T}、\texttt{1B}），為唯一同時攜帶幾何輪廓與樓層資訊之圖層。
    \item \textbf{B5000}（1/5000 基本地形圖，WMS）：以黑色細線繪製建物封閉輪廓，道路與地籍界線則以紅色、行政界以洋紅、水體以藍／青繪製，各要素色彩互斥可分離，作為幾何輪廓之獨立驗證來源。
    \item \textbf{EMAP}（臺灣通用電子地圖，WMS）：建物範圍以淡紫灰色（RGB 234, 227, 234）填色標示，經與 B5000 黑線封閉區域交叉比對後實測確認，作為建物面積之第二項獨立驗證來源。
\end{itemize}

三項服務並非各自獨立使用，而是以同一空間網格（相同 EPSG:3857 邊界框、相同像素尺寸）分別取得柵格圖磚，確保像素級對齊、無需重新取樣，方能在後續步驟中逐像素交叉比對。


\subsection{IoT 微氣象感測器空間資料}
\label{subsec:iot_spatial}

除靜態建物圖資外，本研究整合環境部智慧城鄉空品微型感測器網路之即時觀測資料，作為模擬邊界條件校準與研究場域選定之依據。以研究場域錨點（北緯 24.80 度、東經 120.97 度）為圓心、15 公里為半徑之範圍內，共納入 806 站有效感測器（溫度、濕度量測值均通過合理性篩選），其空間分布與逐時量測特性如圖~\ref{fig:iot_sensor_map} 所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_iot_sensor_map.png}
    \caption{新竹都會區 EPA IoT 微型感測器空間分布實測圖（806 站，2026-07-08 正午時段）}
    \label{fig:iot_sensor_map}
\end{figure}

上圖 (a) 為空氣溫度分布，色階由綠（低溫）至紅（高溫），可見市區與丘陵地帶顯著溫差；(b) 為相對濕度分布，沿海與內陸呈現梯度差異；虛線圓圈為 15 公里資料擷取範圍，十字標示研究場域錨點（新竹市中心）。

在感測器密度分析階段，本研究改以感測器站點之空間座標本身（而非其量測值是否通過合理性篩選）為對象，涵蓋範圍內共 961 站空間位置，並以核密度估計（Kernel Density Estimation, KDE）評估其空間分布密度，以各候選熱點半徑 400 公尺內之實際感測器計數排序，取代單純以 KDE 峰值強度排序 —— 後者對估計頻寬敏感，可能使一處感測器數量稀疏但局部尖峰的候選點排序高於感測器數量真正密集的區域。此一感測器密度分析用以界定後續建物足跡擷取與模型驗證之重點研究範圍，如圖~\ref{fig:studyarea} 所示。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{../urban-thermal-gnn/04_training/figures/fig_studyarea_sensor_density.png}
    \caption{研究範圍選定：IoT 感測器密度與模擬場域對照}
    \label{fig:studyarea}
\end{figure}

上圖 (a) 為 15 公里感測器涵蓋範圍內之核密度估計曲面，虛線圓圈標示以實際感測器計數排序之前五高密度候選區；(b) 為密度最高候選區（即研究場域錨點所在區域）之局部放大，虛線圓圈半徑 400 公尺內共 39 站感測器。
...
\begin{figure}[htbp]
    ...
    \caption{ETH GlobalCanopyHeight 樹冠高度資料與研究場域對照}
...
\subsection{真實場景訓練資料集（V4）}
\label{subsec:v4_real_dataset}

為使模型訓練直接立基於真實都市地理資訊而非全然程序化生成之幾何，本研究於前述 GIS 圖資基礎上，建立一套完全以真實資料驅動之 300 場景訓練資料集（下稱 V4）。其選址以陽明交通大學光復校區為錨點，為達科學可信之取樣數而將搜尋範圍擴及新竹市、新竹縣、桃園市、苗栗縣與臺中市；候選場址直接取自環境部 IoT 測站之真實座標（去重至最小間距 150 公尺，避免 $80\times80$ 公尺場域重疊），並逐一檢核以下四項條件，僅保留同時滿足全部條件者：

\begin{enumerate}
    \item \textbf{真實建物幾何（陰影訊息）：}場域 120 公尺陰影脈絡範圍內至少含 3 棟真實 OSM 建物足跡，且至少 1 棟位於場域中心 70 公尺內，確保場景具真實建物量體與陰影結構；
    \item \textbf{地表覆蓋異質性：}場域周邊具建成（built）以外之至少一類地面材質（草地、水體、裸土、鋪面等，取自 OSM \texttt{landuse}／\texttt{natural}／\texttt{leisure}），合計 $\ge 2$ 類物理表面，提供反照率與蓄熱異質性；
...
全區共 304 個候選場址通過全部條件，依綜合品質評分（真實樓高建物數、IoT 密度、樹冠像元、建物密度加權）取前 300 名，其於真實地圖上之分布如圖~\ref{fig:selected_sites} 所示。300 場景依序輪派至 6、7、8 三個月份（各 100 場景），使資料集涵蓋整個實測夏季。誠實說明各項真實度：建物\textbf{足跡}為 100\% 真實（每場景平均 11.3 棟，直接來自 OSM）；建物\textbf{樓高}則因臺灣 OSM 高度標籤稀疏，僅約 14\% 建物具真實 \texttt{height}／\texttt{building:levels} 標籤，其餘以三層（10.8 公尺）之樓層預設值回填，屬本資料集之已知限制；喬木高度取自 ETH 樹冠實測；空氣溫度與相對濕度則為\textbf{全部 300 場景}各自所在 IoT 測站之真實逐時觀測（溫、濕度覆蓋率均為 300/300）。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_selected_real_sites.png}
    \caption{V4 真實訓練場景選取結果：(a) 300 個場景於擴大搜尋範圍（新竹／桃園／苗栗／臺中，錨定陽明交大光復校區）之區域分布，點色對應綜合選址評分；(b) 錨點核心區之場景，$80\times80$ 公尺場域邊界按真實比例繪製於 OpenStreetMap 底圖之上。每一場景同時滿足真實建物、地表異質性、真實 IoT 站點與真實樹冠四項條件。}
    \label{fig:selected_sites}
\end{figure}


\section{物理式都市微氣候逐時模擬}
\label{sec:ladybug_workflow}

微氣候逐時模擬以一套物理式（Physics-based）Python 運算引擎實作，逐一評估場景內每個空氣節點之逐時熱壓力目標值 $T_a, \text{MRT}, V_a, \text{RH}, \text{UTCI}$。其運算核心包含四個相互耦合之物理模組：（1）\textbf{天空可視因子（Sky View Factor, SVF）}以射線投射法（Ray-casting）自各節點向天空半球發射射線，依建物與喬木幾何計算被遮擋比例；（2）\textbf{陰影判定}依太陽位置（由場域經緯度、日期、時刻以天文公式解算）與建物、喬木幾何投影，逐時判定各節點是否位於遮蔽區；（3）\textbf{戶外平均輻射溫度（MRT）}以地表能量平衡近似式計算，區分遮蔽／曝曬狀態下之短波直達、漫射與地面反射及長波輻射交換（近似參數依 Oke 都市氣候學經驗值設定）；（4）\textbf{街谷風場遮蔽}依建物高寬比（$H/W$）與粗糙度修正參考風速。最終以 \texttt{pythermalcomfort} 套件之標準 UTCI 多項式，由 $T_a$、MRT、$V_a$、RH 合成通用熱氣候指標。無論幾何輸入來自第~\ref{sec:building_extraction}節之真實都市擷取或既有之隨機生成場景，均經由此同一模擬流程取得目標值。模擬時段涵蓋 08:00 至 18:00 共 11 個整點時步（$T = 11$），捕捉日間熱壓力最嚴峻之逐時變化軌跡。

需誠實說明者：本引擎為可重現、可批次執行於數百場景之物理近似模型，並\textbf{未}呼叫 Ladybug Tools／EnergyPlus／Radiance 等外部模擬套件；其 SVF 與陰影為幾何射線近似而非完整輻射傳遞（Radiance）計算，地表能量平衡亦為經驗近似而非 EnergyPlus 之全熱力學求解。此一取捨係為在數百個真實場景之資料集規模下維持運算可行性；引擎所採之能量平衡與經驗係數均可追溯至既有都市氣候文獻，並以真實氣象邊界條件驅動（見下節），其輸出作為機器學習之監督信號在相對意義上具物理一致性。

氣象邊界條件於第~\ref{subsec:v4_real_dataset}節之真實場景資料集中，直接採用各場景所在地之環境部 IoT 測站真實逐時氣溫與相對濕度（經合理性驗證，故障測站退回中央氣象署區域觀測）、中央氣象署測站真實風速，以及區域 TMY（Typical Meteorological Year）EPW 之實測日射剖面（因 CWA 該測站未提供日射觀測，故日射採區域典型氣象年之實測輻射，而非場景當日觀測）；此一設計使模擬之熱壓力目標值錨定於真實觀測氣候。

此一模擬工作流之輸出，除作為機器學習訓練之監督信號外，亦是第~\ref{sec:nsga2}節熱舒適人行動線評估與多目標最佳化之基準比對依據。


\section{運算框架：真實都市 Building Footprint \& Height 擷取}
\label{sec:building_extraction}

\subsection{既有隨機生成方法之侷限}
\label{subsec:parametric_limitation}

模型訓練所需的大規模場景資料集，傳統上由隨機幾何採樣器（Stochastic Geometry Sampler）以蒙地卡羅法在容積率（FAR）、建蔽率（BCR）與縮退規範之法規約束下生成：每個場景包含 2--5 棟建築（矩形或 L 形平面、面寬與進深各 5--25m、樓層數 2--12 層）與 2--5 棵喬木（高度 4--12m），共產生 300 個合法場景。此方法能有效覆蓋法規允許之形態解空間，但其產出之建物量體、棟數關係與街廓紋理均為程序化生成，與真實都市紋理之棟距、街廓尺度、樓高分布統計特性未必一致，使模型於真實場地應用時的泛化可信度難以驗證。因此，本研究另建立一條可與隨機生成互補的真實資料擷取路徑：直接自政府開放圖資擷取真實建物足跡與樓高，作為模型於實際場地驗證與案例研究之幾何輸入來源。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_geo4_scenarios.png}
    \caption{六個典型隨機生成場景之建築量體與喬木配置示意}
    \label{fig:scenarios}
\end{figure}


\subsection{三服務融合建物向量化方法}
\label{subsec:vectorize_method}

真實建物量體之擷取面臨一項核心限制：單一圖資來源難以同時提供精確幾何輪廓與樓層屬性。TOPO01K 圖層雖標註樓高文字，但其柵格化 hatch 填充在高縮放層級下受制圖線寬影響而破碎為大量細小連通元件，直接以連通元件分析取得的建物遮罩並不可靠；B5000 與 EMAP 雖幾何品質較佳，卻不含樓層資訊。本研究因此設計一套三服務交叉驗證方法，讓三個各有所長、各有所缺的圖資來源互補：
...
\subsection{與隨機生成之互補關係}
\label{subsec:extraction_vs_parametric}

真實建物擷取流程與既有隨機生成場景並非互斥替代關係：前者提供真實都市紋理下之案例驗證與 NSGA-II 最佳化之現況基準（Baseline），後者則因場景數量與形態組合可控，適於支撐模型訓練所需之大規模、高覆蓋度樣本集。第~\ref{chapter:results}章之案例研究即以本節擷取之真實建物足跡作為既有場地現況輸入，評估模型於未見過真實幾何配置下之泛化表現。
...
建物節點共 $N_{\text{obj}}$ 個，包含場景內所有建築與喬木，每個節點攜帶 7 維靜態幾何特徵向量（見表~\ref{tab:obj_features}）：
...
式中 $\text{GHI}$（W/m$^2$）為太陽總輻照度（來自 EPW 逐時資料），$h_c$（W/m$^2$K）為對流換熱係數（本研究設定為 10 W/m$^2$K）。此估算方法在忽略長波輻射再輻射與儲熱效應的一階近似下，能有效代表城市鋪面之熱輻射貢獻 \citep{oke1988street}。


\subsection{邊緣類型與因果拓樸}
\label{subsec:edges}

本研究定義五種關係型邊緣（Relational Edges），各自對應一種明確的物理因果機制（見表~\ref{tab:edge_types}）：
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        \texttt{shadow}      & 建物 $\to$ 空氣 & 幾何陰影投射計算 & 建築遮擋導致輻射熱降低 \\
        \texttt{veg\_et}     & 建物 $\to$ 空氣 & 樹冠覆蓋半徑判斷 & 植被蒸散作用形成冷卻效應 \\
        \texttt{convective}  & 空氣 $\to$ 空氣 & 對流方向性權重   & 風場驅動的對流熱交換 \\
        \texttt{semantic}    & 建物 $\leftrightarrow$ 建物 & 全連接（完整關係） & 建築語義形態相互關係 \\
        \texttt{contiguity}  & 空氣 $\leftrightarrow$ 空氣 & KNN（$k = 8$）  & 空間毗鄰性熱梯度傳導 \\
        \bottomrule
    \end{tabular}
\end{table}

其中 \texttt{semantic} 與 \texttt{contiguity} 為靜態邊，於資料載入階段一次建立並全程共用；\texttt{shadow}、\texttt{veg\_et}、\texttt{convective} 三者則隨太陽方位角、風向逐時步變化，於每個時步重新計算，構成動態邊集合 \texttt{dynamic\_edges[t]}。圖~\ref{fig:graph_construction} 以真實資料集之場景 0（$N_{\text{obj}}=3$，$N_{\text{air}}=6{,}241$）具體呈現此一異質圖之節點分布、真實建立之 KNN 毗鄰邊局部結構，以及完整節點／邊型別綱要。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_graph_construction.png}
    \caption{異質圖建構實例（取自真實訓練資料場景 0）}
...
圖~\ref{fig:graph_topology} 進一步以另一真實場景（場景 10）之全域視角，呈現 \texttt{semantic} 邊如何在場域內所有建物節點間建立全連接拓樸——不同於圖~\ref{fig:graph_construction}(b) 僅呈現單一建物之局部窗口，此處可清楚看見四個建物節點（含一喬木冠層節點）兩兩相連之完整語義邊網絡，與其下方 $6{,}241$ 個空氣節點所構成之密集 KNN 毗鄰網格形成尺度對比。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_geo5_graph_topology.png}
    \caption{異質圖拓樸全域視角（真實訓練資料場景 10）}
    \label{fig:graph_topology}
\end{figure}

上圖 (a) 為完整 $80\times80$ 公尺場域，橘點為 6,241 個空氣節點（KNN 毗鄰邊為求可讀性僅隨機顯示 3,000／49,928 條），藍色量體為建物節點、綠色圓為喬木冠層節點，紫色虛線為建物節點間之全連接語義邊；(b) 為場域左下 $40\times40$ 公尺象限放大，標示 9 維空氣節點特徵之完整定義。

作為對照，圖~\ref{fig:real_scene_graph} 以第~\ref{subsec:v4_real_dataset}節 V4 真實場景資料集中之一個實際場景，展示完全以真實 GIS 圖資建構之異質圖：(a) 之建物量體均為 OSM 真實足跡（色階為真實簷高，實線外框標示具真實樓高標籤者），綠點為 ETH 樹冠實測植栽；(b) 之每個空氣節點依 4 公尺格點佈設，節點顏色為以該場景所在真實 IoT 測站逐時氣溫驅動之模擬尖峰 UTCI，可見建物遮蔽與植栽降溫在空間上形成之熱舒適梯度，即為 PI-ST-GNN 所學習之真實空間結構。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_real_scene_graph.png}
    \caption{以真實 GIS 圖資建構之異質圖範例（V4 真實場景）：(a) 真實 OSM 建物與 ETH 樹冠幾何；(b) 空氣節點感測網格與 k-NN 鄰接邊，節點色為真實 IoT 氣溫驅動之模擬尖峰 UTCI。}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../urban-thermal-gnn/04_training/figures/fig_overview_pistgnn_clean.png}
    \caption{PI-ST-GNN 模型架構總覽}
...
其中 $\mathbf{W}_{\text{self}}$ 為自迴路權重矩陣、$\mathbf{W}_r$ 為關係 $r$ 專屬投影矩陣（本研究 $|\mathcal{R}| = 5$）、$\mathcal{N}_r(i)$ 為節點 $i$ 在關係 $r$ 下的鄰居集合、$\sigma(\cdot)$ 為 PReLU 激活函數，末項為殘差連接並於激活後接 LayerNorm 與 Dropout（$p=0.1$）。模型採用三層堆疊（$L=3$）的 RGCNBlock，允許訊息在三跳鄰域內擴散，有效涵蓋 $80\times80$ 公尺場地內建築陰影的最大投射距離；各關係矩陣之完整維度設定見附錄~\ref{appx:rgcn_detail}。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{../urban-thermal-gnn/04_training/figures/fig_rgcn_detail.png}
    \caption{RGCN 模組訊息聚合機制示意圖}
...
其中 $\mathcal{L}_{\text{data}}$ 為資料驅動項（與 EnergyPlus 模擬標籤之 MSE 損失），$\alpha_1, \alpha_2, \alpha_3$ 為物理損失權重係數。三項物理約束各自對應之空間、時間、風場作用機制與懲罰觸發條件，統整於圖~\ref{fig:physics_loss}；各懲罰項之完整符號定義與數值設定說明如下三小節，其嚴謹解析式推導則置於附錄~\ref{appx:physics_loss_derivation}。
...
城市物質（混凝土、瀝青鋪面）具有顯著的熱儲存效應，使 UTCI 在相鄰時步間的變化率受到自然物理限制——熱壓力不應在一小時內劇烈跳躍。本研究將允許之最大 UTCI 變化率設定為 $5^\circ\text{C/hr}$（正規化空間中 $\delta_{\text{temp}} = 0.625$，其換算依據見附錄~\ref{appx:physics_loss_derivation}）：
...
其中 $\mu_{\hat{u}}$、$\sigma_{\hat{u}}$ 分別為當前時步全場域預測 UTCI（所有節點，非僅 $\mathcal{B}_{\text{tall}}$）之均值與標準差，懲罰同樣採線性鉸鏈形式。閾值 $\mu_{\hat{u}} + 1.5\sigma_{\hat{u}}$ 之選定依循統計離群偵測之常見慣例——在常態分布假設下，超出均值 $1.5$ 個標準差約對應最熱的 6.7\% 尾端節點，將「熱死角」操作型定義為場域中溫度顯著偏離常態分布中心之高層建築鄰近節點。此約束強制模型學習高層建築背風側之熱累積模式，使設計師能可視化熱死角位置，進而優化建築間距與通風廊道配置。


\section{Grasshopper 介面整合與多目標最佳化：熱舒適人行動線設計}
\label{sec:nsga2}

\subsection{從最短路徑到熱舒適動線評估}
\label{subsec:from_astar_to_walkway}

早期構想以 $A^*$ 最短路徑演算法將 UTCI 熱壓力場轉譯為單一起訖點間之熱迴避路徑：以節點間歐氏距離與熱壓力懲罰之加權組合作為邊成本，求解一條在行走效率與熱舒適間取捨之路徑。然而，都市開放空間設計決策的對象通常並非單一使用者的單次行程，而是整體人行動線網絡在一整日各時段的熱舒適表現；單點對單點的最短路徑求解無法回答「這個街廓的人行動線系統整體而言是否宜行」此一設計層級問題。本研究因此將原先的路徑搜尋機制，重新定位為\textbf{熱舒適人行動線設計}（Thermal Comfort-Based Walkway Design）評估指標，直接整合進 NSGA-II 多目標最佳化之適應度函數，而非作為獨立之單次路徑查詢工具。
...
其中 $E_{\text{walk}}$ 為人行動線網絡之邊集合、$D_{ij}$ 為邊之實際步行距離、$\Phi(\cdot)$ 為無量綱熱壓力懲罰函數，其定義與完整推導見附錄~\ref{appx:walkway_derivation}。此一時空平均指標取代原先僅評估單一路徑之作法，使評估對象與 NSGA-II 之全域最佳化目標在尺度上一致。


\subsection{Grasshopper 介面與非同步通訊架構}
\label{subsec:gh_integration}

系統採用\textbf{非同步用戶端-伺服器架構}，讓建築師能在熟悉的設計環境中獲得 AI 的即時反饋：在 Grasshopper 中開發之專用套件（\texttt{UTCIPredictor}、\texttt{UTCIOptimizer}）負責將 Rhino 3D 幾何物件序列化為異質圖相容張量；後端透過 IETF RFC 6455 \textbf{WebSocket} 通訊協定 \citep{rfc6455} 與 FastAPI 通訊，載入預先訓練完成的 PI-ST-GNN 模型權重，於數十毫秒內回傳預測結果與優化進度；設計師於 Rhino 中移動建築時，系統立即更新場地 1.0m 間距之高解析度熱舒適度網格，將傳統耗費大量計算的 CFD 模擬轉化為即時的設計適應回饋 \citep{sadeghipour2013ladybug}。
...
\begin{figure}[htbp]
    ...
    \caption{Grasshopper--FastAPI／WebSocket 非同步通訊架構時序圖}
...
優化引擎將都市配置參數化為連續染色體基因組，所有變數於運算前歸一至 $[0,1]$ 區間：每棟建築由質心座標 $(c_x, c_y)$、面寬 $w$、進深 $d$、平面旋轉角度與樓層數共六項參數定義；每棵喬木由座標 $(x,y)$、樹冠半徑與高度四項參數組成，直接決定遮蔭效果與綠化率貢獻。

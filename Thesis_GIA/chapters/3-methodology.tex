...
微氣候逐時模擬以一套物理式（Physics-based）Python 運算引擎實作，逐一評估場景內每個空氣節點之逐時熱壓力目標值 $T_a, \text{MRT}, V_a, \text{RH}, \text{UTCI}$。其運算核心包含四個相互耦合之物理模組：

\begin{enumerate}
    \item \textbf{天空可視因子（Sky View Factor, SVF）}：以射線投射法（Ray-casting）自各節點向天空半球發射射線，依建物與喬木幾何計算被遮擋比例。
    \item \textbf{陰影判定}：依太陽位置（由場域經緯度、日期、時刻以天文公式解算）與建物、喬木幾何投影，逐時判定各節點是否位於遮蔽區。
    \item \textbf{戶外平均輻射溫度（MRT）}：以地表能量平衡近似式計算，區分遮蔽／曝曬狀態下之短波直達、漫射與地面反射及長波輻射交換（近似參數依 Oke 都市氣候學經驗值設定）。
    \item \textbf{街谷風場遮蔽}：依建物高寬比（$H/W$）與粗糙度修正參考風速。
\end{enumerate}

最終以 \texttt{pythermalcomfort} 套件之標準 UTCI 多項式，由 $T_a$、MRT、$V_a$、RH 合成通用熱氣候指標。無論幾何輸入來自第~\ref{sec:building_extraction}節之真實都市擷取或既有之隨機生成場景，均經由此同一模擬流程取得目標值。模擬時段涵蓋 08:00 至 18:00 共 11 個整點時步（$T = 11$），捕捉日間熱壓力最嚴峻之逐時變化軌跡。

需說明者：本引擎為可重現、可批次執行於數百場景之物理近似模型，並\textbf{未}呼叫 Ladybug Tools／EnergyPlus／Radiance 等外部模擬套件；其 SVF 與陰影為幾何射線近似而非完整輻射傳遞（Radiance）計算，地表能量平衡亦為經驗近似而非 EnergyPlus 之全熱力學求解。此一取捨係為在數百個真實場景之資料集規模下維持運算可行性；引擎所採之能量平衡與經驗係數均可追溯至既有都市氣候文獻，並以真實氣象邊界條件驅動（見下節），其輸出作為機器學習之監督信號在相對意義上具物理一致性。

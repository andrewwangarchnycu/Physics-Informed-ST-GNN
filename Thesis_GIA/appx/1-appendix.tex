...
換言之，本研究實際訓練並報告之模型，其時序平滑約束在原始溫度尺度上生效的容許變化率約為 $1.87\,^\circ\text{C/hr}$，較正文所述之設計目標 $5\,^\circ\text{C/hr}$ 嚴格約 2.7 倍。此一落差源於 $\delta_{\text{temp}}$ 設定時所採用之標準差假設值與訓練資料實測值不一致，特此揭露；由於本研究之全部量化結果（第~\ref{chapter:experiments}章）均基於 $\delta_{\text{temp}}=0.625$ 之模型訓練與評估，此處不回溯更動已訓練模型之設定，而將此列為後續研究應修正之已知實作落差（見第~\ref{sec:limitations}節）。
...
第~\ref{subsec:edges}節表~\ref{tab:edge_types}所描述之五類關係型邊緣，於本論文寫作過程中發現其中三類（\texttt{shadow}、\texttt{veg\_et}、\texttt{convective}）在原始訓練流程中實際上從未被真正建立過。本附錄記錄此一缺陷之發現過程、根本原因、修正方法，以及修正後重新訓練之結果，供讀者核實表~\ref{tab:main_results}、圖~\ref{fig:gnnexplainer}等處數值之來源。
...
第四章消融實驗（表~\ref{tab:ablation}）之 A0、A3--A6 五個變體，其原始訓練早於本次修正，圖結構部分同樣僅具 \texttt{semantic}／\texttt{contiguity} 兩類靜態邊；本論文完稿前已依本附錄所述方法，於真實場景資料集（動態邊已修正）上重新訓練該五個變體，表~\ref{tab:ablation}與相應內文已更新為重新訓練後之結果。第四章圖~\ref{fig:scatter_test}、圖~\ref{fig:hourly_r2}、圖~\ref{fig:confusion}、圖~\ref{fig:training_curves} 亦均為本研究最終採用之 V5-300 模型（動態邊已修正）之直接推論輸出，不涉及本節所述之程序化主線缺陷。


\subsection{真實場景資料集（V1--V4）之同類缺陷與 V5 修正}
\label{appx:v5_dynamic_edges_fix}

上述缺陷之發現與修正原先針對本研究之程序化主線（第~\ref{sec:evaluation}節，\texttt{checkpoints\_v2\_dynedges}）。本節記錄同一類缺陷亦存在於本研究另一條真實場景資料集訓練線（V1--V4，第~\ref{sec:v4_validation}節），以及最終於 V5-300 一併修正之完整過程，供讀者核實表~\ref{tab:main_results}、圖~\ref{fig:gnnexplainer}等處數值之來源。

\paragraph{問題發現} 於程序化主線之缺陷確認後，本研究進一步檢視真實場景資料集訓練線之對應資料前處理流程，發現其動態邊快取（\texttt{dynamic\_edge\_cache.h5}）同樣從未針對真實場景實際建立——V1--V4 之訓練與評估自始至終皆未真正產出此檔案，故 V1--V4 之異質圖訊息傳遞亦僅透過 \texttt{semantic}、\texttt{contiguity} 兩類靜態邊進行，與程序化主線修正前之狀態同屬一類缺陷。V4 於第~\ref{sec:v4_validation}節所述之表面測試集數值（$R^2 \approx 0.99$ 量級），即在此一未修正狀態下取得，不具方法完整性，故本論文不將其作為有效驗證結果引用。

\paragraph{修正方法} 本研究新增動態邊快取重建腳本 \texttt{01\_data\_generation/scripts/16\_build\_dynamic\_edge\_cache\_v5.py}，邏輯上與程序化主線所用之 \texttt{11\_build\_dynamic\_edge\_cache.py} 一致（重建陰影／樹冠覆蓋之來源物件索引），但額外針對真實場景資料集之特性強化兩點：其一，太陽方位角逐場景以其真實經緯度與所屬月份代表日期實際解算，而非沿用單一場域之共用太陽路徑；其二，風向資料改採中央氣象署新竹站（C0D660）之真實逐月觀測風向，取代程序化主線所用之單一典型日風向。此一動態邊快取修正，與第~\ref{sec:ladybug_workflow}節所述模擬引擎由自製近似式全面替換為 Radiance／EnergyPlus 官方流程，共同構成 V4 至 V5 之版本轉變。

\paragraph{修正後結果} 以修正後之真實場景資料集（210／45／45 場景訓練／驗證／測試切分）重新訓練，最佳驗證權重取自第 119 個 Epoch（驗證 $R^2=0.9837$），測試集評估結果（166,188 預測點次）為 $R^2=0.9753$、RMSE$=0.833\,^\circ\text{C}$、MAE$=0.524\,^\circ\text{C}$、熱壓力等級分類準確率 93.9\%，儲存於 \texttt{checkpoints\_v5\_300/}，即為本研究最終採用之主要報告模型 V5-300。圖~\ref{fig:gnnexplainer}(c) 之 GNNExplainer 關係權重范數分析顯示，五類關係之范數皆落於相近量級、無任何一類呈現「未收到真實梯度訊號」之殘留初始值型態，此為動態邊缺陷已修正之直接可視化證據。


\section{消融實驗數值精度細節}
\label{appx:ablation_numeric_detail}

本附錄記錄第~\ref{subsec:ablation_discussion}節消融結果之訓練工程層面細節，供有意深入理解模型訓練行為之讀者查閱；正文僅摘錄與設計輔助宗旨直接相關之結論。

\subsection{純 MLP 基準線（A0）數值優勢之成因}

不具空間圖結構的純 MLP（A0）於 $R^2$、RMSE 兩項數值指標上均優於本研究之完整提案架構，其成因有三：其一，UTCI 之主要變異量很大程度由太陽仰角（時間）與節點座標（位置）直接決定，此一高度規律性的映射任務本身即適合 MLP 之全局統計擬合，且 MLP 參數量遠低於 RGCN+LSTM，於 210 個真實訓練場景之有限樣本規模下相對不易過擬合；其二，A0 缺乏物理約束之預測誤差對整體 RMSE 的拉抬有限（其對建築設計決策構成之誤導風險，詳見正文第~\ref{subsec:ablation_discussion}節）；其三，RGCN 的訊息聚合機制引入鄰域平均效應，相當於一種空間正則化，於 Radiance／EnergyPlus 模擬所呈現之更劇烈局部熱極端值情境下（如背風側熱滯留或高反照率鋪面周邊之 MRT 尖峰，第~\ref{sec:v4_validation}節），此一空間平滑效應對峰值之低估較程序化近似式資料更為顯著，構成 RMSE 上升之額外來源。

\subsection{A6 與 V5-300 官方 checkpoint 之數值差異}

本節消融實驗中獨立重跑之完整模型變體（A6）與表~\ref{tab:main_results}之 V5-300 官方主要報告模型，雖架構與訓練資料完全相同，數值仍有可辨識差異（A6：$R^2=0.9655$；V5-300：$R^2=0.9753$）。此屬兩次獨立訓練執行（未固定相同隨機種子）於 210 個真實訓練場景之有限樣本規模下的正常訓練變異，而非方法不一致；表~\ref{tab:main_results}、圖~\ref{fig:training_curves}--\ref{fig:spatial_comparison}與本論文其餘各章節，均以官方 V5-300 checkpoint 為準。
